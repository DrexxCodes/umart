...
import { formatMonthLabel, getCurrentMonthId } from '@/hooks/useDateData'
...
async function fetchCustomDocs(ids: string[]): Promise<{ docs: AnalyticsDoc[]; global: GlobalDoc }> {
  const params = new URLSearchParams({ period: 'monthly', start: ids[0], end: ids[ids.length-1] })
  const res = await fetch(`/api/admin/analytics?${params}`)
  if (!res.ok) throw new Error(`HTTP ${res.status}`)
  const json = await res.json()
  return {
    docs:   (json.monthly  ?? []) as AnalyticsDoc[],
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
        ...
        <StatCard label="Monthly Platform Fee"     value={current.totalPlatformFee} currentValue={current.totalPlatformFee} previousValue={previous?.totalPlatformFee} isCurrency accent="destructive" />
        {showGlobal && (
...
export function Monthly({ docs, global, loading }: MonthlyProps) {
  ...
  const defaultChart: ChartDataPoint[] = sorted
    ...
    .map(d=>({ label:formatMonthLabel(d.id), usersSignedUp:d.usersSignedUp, totalPaid:d.totalPaid, totalWithdrawn:d.totalWithdrawn, productsCreated:d.productsCreated }))
  ...
  const renderContent = () => {
    if (!selection) return (
      <>
        ...
        <ChartBlock data={defaultChart} title="This month vs last month"/>
      </>
    )
    ...
    const customChart: ChartDataPoint[] = sortedCustom.map(d=>({ label:formatMonthLabel(d.id), usersSignedUp:d.usersSignedUp, totalPaid:d.totalPaid, totalWithdrawn:d.totalWithdrawn, productsCreated:d.productsCreated }))
    ...
    if (selection.mode === 'view') {
      ...
      return (
        <>
          ...
          <ChartBlock data={customChart} title={formatMonthLabel(doc.id)}/>
        </>
      )
    }
    ...
    return (
      <>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="rounded-xl border border-border bg-card p-4"><StatGrid current={docA} label={`A · ${formatMonthLabel(docA.id)}`} globalFee={customGlobal.totalPlatformFee}/></div>
          <div className="rounded-xl border border-border bg-card p-4"><StatGrid current={docB} label={`B · ${formatMonthLabel(docB.id)}`} globalFee={customGlobal.totalPlatformFee}/></div>
        </div>
        <div className="rounded-xl border border-primary/15 bg-primary/5 p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            ...
            <StatCard label="Monthly Platform Fee"     value={docB.totalPlatformFee} currentValue={docB.totalPlatformFee} previousValue={docA.totalPlatformFee} isCurrency accent="destructive" />
          </div>
        </div>
        <ChartBlock data={customChart} title={`${formatMonthLabel(docA.id)} vs ${formatMonthLabel(docB.id)}`}/>
      </>
    )
  }
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      {showTimeline && (
        <div className="max-w-sm">
          <TimelinePicker period="monthly" onSearch={handleSearch} onClear={handleClear} loading={timelineLoading}/>
        </div>
      )}
    </div>
  )
